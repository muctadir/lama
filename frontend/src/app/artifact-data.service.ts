...
export class ArtifactDataService {
  ...
  getArtifacts(p_id: number): Promise<Array<StringArtifact>> {
    ...
    // Actual request
    return axios
      .get('http://127.0.0.1:5000/artifact/artifactmanagement', {
        headers: {
          u_id_token: token,
        },
        params: {
  ...
  addArtifacts(
    ...
    if (artifacts.length <= 0)
      throw new Error('No artifacts have been submitted');
    // Check if the artifacts are empty
    for (const element of artifacts) {
      if (Object.keys(element).length <= 0)
        throw new Error('Artifacts cannot have empty fields');
    }

    let result: boolean;
    // Send the data to the database
    return axios
      .post('http://127.0.0.1:5000/artifact/creation', artifacts, {
        headers: {
          u_id_token: token,
        },
        params: {
  ...
  getArtifact(p_id: number, a_id: number): Promise<Array<any>> {
    ...
    return axios
      .get('http://127.0.0.1:5000/artifact/singleArtifact', {
        headers: {
          u_id_token: token,
        },
        params: {
      ...
      .then((response) => {
        ...
        return [
          result,
          response.data['artifact_labellings'],
