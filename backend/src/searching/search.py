...
#@returns a dictionary per artifact id of value {'id', 'dist', 'word', 'search'}
...
def dickify(param_list):
    ...
    for i in param_list:
        ...
        art_word = i[1][1]
        search_word = i[2]
        new_list.append({'id':art_id,'dist':art_dist,'word':art_word, 'search':search_word})
...
def best_search_results(all_search_results, word_count):
    ...
    for result in all_search_results:
        #print("len of results", len(all_search_results[result]), word_count)
...
results = search_func_all_res(search_words, data)
best_search_results(results, search_word_count)
